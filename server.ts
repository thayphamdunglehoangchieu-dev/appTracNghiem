...
// API KEY STATUS INTERFACE & MANAGER FOR ROTATION AND STATUS ALERTS
...
class ApiKeyManager {
  ...
  private configPath = path.join(process.cwd(), "api_keys.json");
  ...
  public loadKeys() {
    ...
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        if (Array.isArray(parsed.keys)) {
          for (let i = 0; i < 4; i++) {
            if (parsed.keys[i]) {
              loadedKeys[i] = String(parsed.keys[i]).trim();
            }
          }
          console.log("[ApiKeyManager] Đã tải keys từ api_keys.json");
        }
      }
    } catch (err) {
    ...
    // 2. If all keys are empty, try env variables
    const allEmpty = loadedKeys.every(k => k === "");
    if (allEmpty) {
      const envKeys = [
      ...
      if (envKeys.every(k => k === "") && mainKey !== "") {
        const splitKeys = mainKey.split(/[\s,;]+/).filter(k => k.trim() !== "");
        for (let i = 0; i < 4; i++) {
          if (splitKeys[i]) {
            envKeys[i] = splitKeys[i].trim();
          }
        }
      }
      ...
      for (let i = 0; i < 4; i++) {
        if (envKeys[i]) {
          loadedKeys[i] = envKeys[i].trim();
        }
      }
      if (!loadedKeys.every(k => k === "")) {
        console.log("[ApiKeyManager] Đã tải keys từ biến môi trường (.env)");
      }
    }
    ...
    this.keyStatuses = this.keys.map((key, index) => ({
      key,
      index,
      status: key ? "active" : "unconfigured",
      errorMessage: undefined,
      lastUsed: undefined
    }));

    // Find first active index
    this.currentIndex = this.keyStatuses.findIndex(ks => ks.status === "active");
    if (this.currentIndex === -1) this.currentIndex = 0;
  }

  public saveKeys(newKeys: string[]) {
    this.keys = newKeys.map(k => k.trim());
    try {
      fs.writeFileSync(this.configPath, JSON.stringify({ keys: this.keys }, null, 2), "utf-8");
      console.log("[ApiKeyManager] Đã lưu keys vào api_keys.json");
    } catch (err) {
      console.error("[ApiKeyManager] Lỗi ghi file api_keys.json:", err);
    }

    // Reset status and start index
    this.keyStatuses = this.keys.map((key, index) => ({
      key,
      index,
      status: key ? "active" : "unconfigured",
  ...
  public getStatuses() {
    return this.keyStatuses.map(ks => ({
      index: ks.index,
      status: ks.status,
      errorMessage: ks.errorMessage,
      lastUsed: ks.lastUsed,
      keyMasked: ks.key ? `${ks.key.slice(0, 6)}...${ks.key.slice(-4)}` : "Chưa cấu hình"
    }));
  }
  ...
  public getActiveClient(): { client: GoogleGenAI; index: number } | null {
    let attempts = 0;
    while (attempts < 4) {
      const idx = (this.currentIndex + attempts) % 4;
      const ks = this.keyStatuses[idx];
      if (ks && ks.status === "active" && ks.key) {
        ks.lastUsed = new Date().toISOString();
        ...
        // Update current pointer to the selected active key
  ...
  public markAsExhausted(index: number, errorMsg: string) {
    if (index >= 0 && index < 4) {
      this.keyStatuses[index].status = "exhausted";
      this.keyStatuses[index].errorMessage = errorMsg;
      console.warn(`[ApiKeyManager] API Key #${index + 1} hết hạn ngạch/token: ${errorMsg}`);
    }
  }

  public markAsExpiredOrInvalid(index: number, errorMsg: string) {
    if (index >= 0 && index < 4) {
      this.keyStatuses[index].status = "expired";
      this.keyStatuses[index].errorMessage = errorMsg;
      console.warn(`[ApiKeyManager] API Key #${index + 1} hết hạn/sai khóa: ${errorMsg}`);
    }
  }
  ...
  public resetStatuses() {
    this.keyStatuses.forEach(ks => {
      if (ks.key) {
        ks.status = "active";
        ks.errorMessage = undefined;
      } else {
        ks.status = "unconfigured";
      }
    });
    this.currentIndex = this.keyStatuses.findIndex(ks => ks.status === "active");
    if (this.currentIndex === -1) this.currentIndex = 0;
...
// Wrapper for API Key rotation and tracking
async function generateContentWithRotation(
  ...
  // 1. Get current active client
  const activeClientInfo = apiKeyManager.getActiveClient();
  if (!activeClientInfo) {
    throw new Error("Không tìm thấy API Key nào đang hoạt động. Vui lòng kiểm tra lại cấu hình API Keys trong phần cài đặt!");
  }
  
  const { client, index } = activeClientInfo;
  console.log(`[Key Rotation] Sử dụng API Key #${index + 1} để gọi Gemini...`);
  ...
  try {
    ...
    const errMsg = err.message || String(err);
    console.error(`[Key Rotation] Gặp lỗi với API Key #${index + 1}:`, errMsg);
    ...
    if (isQuotaExceeded) {
      const logMsg = `API Key số ${index + 1} bị hết hạn ngạch (Quota Exceeded). Đang tự động chuyển key.`;
      logs.push(logMsg);
      apiKeyManager.markAsExhausted(index, errMsg);
    } else if (isInvalidKey) {
      const logMsg = `API Key số ${index + 1} bị lỗi hết hạn hoặc không hợp lệ. Đang tự động chuyển key.`;
      logs.push(logMsg);
      apiKeyManager.markAsExpiredOrInvalid(index, errMsg);
    } else {
      const logMsg = `API Key số ${index + 1} gặp lỗi: ${errMsg.slice(0, 80)}. Đang tự động chuyển key.`;
      logs.push(logMsg);
      apiKeyManager.markAsExhausted(index, errMsg); // Mark as exhausted for safety
    }
    
    // Rotate key and retry
    if (rotationsAttempted < 3) {
      console.log(`[Key Rotation] Đang xoay vòng sang API Key tiếp theo...`);
      apiKeyManager.rotateToNext();
      const retryResult = await generateContentWithRotation(model, contents, config, rotationsAttempted + 1);
      return {
        response: retryResult.response,
        logs: [...logs, ...retryResult.logs]
      };
    } else {
      throw new Error(`Cả 4 API Keys đều gặp lỗi hoặc hết hạn ngạch. Lỗi cuối cùng: ${errMsg}`);
    }
  }
}
...
Nhiệm vụ của bạn là trích xuất và trả về một mảng gồm đúng 12 câu hỏi trắc nghiệm thô từ tài liệu của giáo viên theo định dạng JSON. Không cần lo lắng về định dạng LaTeX trong bước này.`;
    ...
    if (clientApiKey && clientApiKey.trim() !== "") {
      ...
      // Rotation fallback mode
      const activeKeysCount = apiKeyManager.getRawKeys().filter(k => k !== "").length;
      if (activeKeysCount === 0) {
        if (currentStep === null || currentStep === 1) {
          ...
          return res.json({
            ...
            quiz: draftQuiz,
            isFallback: true,
            warning: "Vui lòng nhập API Key của bạn qua nút Settings trên Header hoặc cấu hình trong tab Cấu hình API Keys để sử dụng AI. Hệ thống tạm thời hiển thị Đề thi mẫu!"
          });
        } else {
...
app.get("/api/keys", (req, res) => {
  res.json({
    success: true,
    keys: apiKeyManager.getStatuses(),
    rawKeys: apiKeyManager.getRawKeys() // Send raw keys (we'll only show masked in regular fields, but allow editing in inputs)
  });
});

app.post("/api/keys/save", (req, res) => {
  const { keys } = req.body;
  if (!keys || !Array.isArray(keys) || keys.length !== 4) {
    return res.status(400).json({ success: false, error: "Dữ liệu API keys không hợp lệ (phải có đúng 4 keys)." });
  }
  apiKeyManager.saveKeys(keys);
  res.json({
    success: true,
    message: "Đã lưu và kích hoạt cấu hình API Keys mới.",
    keys: apiKeyManager.getStatuses()
  });
});

app.post("/api/keys/reset", (req, res) => {
  apiKeyManager.resetStatuses();
  res.json({
    success: true,
    message: "Đã thiết lập lại trạng thái hoạt động cho tất cả các API Keys.",
    keys: apiKeyManager.getStatuses()
  });
});
